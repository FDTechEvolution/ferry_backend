...
@section('content')
    <div class="row">
        <div class="col-12">
            <a href="{{ route('api.edit',['id'=>$apiMerchant->id]) }}" class="btn btn-secondary"><i class="fa-solid fa-circle-left"></i> Back to {{$apiMerchant->name}} API Setting</a>
        </div>
    </div>
    <hr>
    <div class="row">
                            ...
                            <select class="form-select" id="api_route_id" name="api_route_id" aria-label=""
                                ...
                                @foreach ($apiMerchant->apiRoutes as $apiRoute)
                                    <option value="{{ $apiRoute->pivot->id }}" @selected($apiRoute->pivot->id == $apiRouteId)>
                                        {{ sprintf('%s > %s : %s/%s', $apiRoute->station_from->name, $apiRoute->station_to->name, date('H:i', strtotime($apiRoute->depart_time)), date('H:i', strtotime($apiRoute->arrive_time))) }}
                                    </option>
                                @endforeach
                            </select>
                                    ...
                                    @foreach ($row as $col)
                                        <td class="text-end">
                                            ...
                                            <span>
                                                ...
                                                <a href="#" data-href="{{route('routeCalendar.create',['api_route_id'=>$apiRouteId,'date'=>$col['date']])}}"
                                                data-ajax-modal-size="modal-md"
                                                data-ajax-modal-centered="true"
                                                data-ajax-modal-callback-function=""
                                                data-ajax-modal-backdrop="static" class="js-ajax-modal">
                                                    Max seat <strong>{{ number_format($col['seat']) }} <i
                                                            class="fa-solid fa-pen-to-square"></i></strong>
                                                </a>
                                                @else
                                                <a  href="#" data-href="{{route('routeCalendar.edit',['routeCalendar'=>$col['id']])}}"
                                                    data-ajax-modal-size="modal-md"
                                                data-ajax-modal-centered="true"
                                                data-ajax-modal-callback-function=""
                                                data-ajax-modal-backdrop="static" class="js-ajax-modal text-warning">
                                                    Max seat <strong>{{ number_format($col['seat']) }} <i
                                                            class="fa-solid fa-pen-to-square"></i></strong>
                                                </a>
                                                @endif
