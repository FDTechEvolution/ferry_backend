                            ...
                            @foreach ($users as $index => $user)
                                <tr class="">
                                    ...
                                    <td class="">
                                        ...
                                        <input type="hidden" id="role-{{ $index }}"
                                            value="{{ $user['role_id'] }}">
                                        <input type="hidden" id="image-{{ $index }}" value="{{ $user['image'] }}">

                                        <!-- edit -->
                                        <x-action-edit class="me-2" :url="_('javascript:void(0)')" id="btn-user-edit"
                                            onClick="updateEditData({{ $index }})" />

                                        <!-- delete -->
                                        <x-action-delete :url="route('user-delete', ['id' => $user['id']])" :message="_('Are you sure? Delete this account : ' . $user['username'] . '?')" />
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
