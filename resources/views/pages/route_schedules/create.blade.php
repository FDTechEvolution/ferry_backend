                ...
                <div class="col-12">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input form-check-input-success" type="radio" name="type"
                            id="type_open" value="OPEN" checked>
                    ...
                    <div class="form-floating mb-3">
                        <select class="form-select" id="station_from_id" name="station_from_id" aria-label="">
                            ...
                            @foreach ($stationFroms as $station)
                                <option value="{{ $station->id }}" @selected($stationFromId == $station->id)>{{ $station->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-floating mb-3">
                        <select class="form-select" id="station_to_id" name="station_to_id" aria-label="">
                            ...
                            @foreach ($stationTos as $station)
                                <option value="{{ $station->id }}" @selected($stationToId == $station->id)>{{ $station->name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <script>
        $(document).ready(function() {
            $('#station_from_id').on('change', function() {
                $('#page-loader').show();
                $('#station_to_id').empty();
                $('#_station_from_id').val($('#station_from_id').val());
                $('#frm-search').submit();
            });

            $('#station_to_id').on('change', function() {
                $('#page-loader').show();
                $('#_station_to_id').val($('#station_to_id').val())
                $('#frm-search').submit();
            });
        });
    </script>
