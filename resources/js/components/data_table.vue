<template>
    <table class="table-datatable table table-bordered table-hover table-striped"
        ...
        data-custom="true"
        data-custom-config='{"paging":true, "searching":true}'>
        <thead>
            <tr>
                <th v-for="th in _thead">{{ th }}</th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="td in _tbody">
                <td>{{ td.name }}</td>
                <td>{{ td.age }}</td>
                <td>{{ td.position }}</td>
                <td>{{ td.office }}</td>
...
<script setup>
    const props = defineProps(['thead', 'tbody'])
    const _thead = props.thead
    const _tbody = props.tbody
</script>