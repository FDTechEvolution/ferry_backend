...
class StationInfomationsController extends Controller
{
    ...
    {
        $info = StationInfomation::orderBy('isactive','ASC')->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($info) {
            if ($request->station_id !== '')
                $this->setInfoToStation($info->id, $request->station_id, $request->master_info);
    ...
    public function update(Request $request)
    {
        ...
        if (!$this->checkInfoName($request->name, $request->type, $request->station_info_id))
            return redirect()->route('stations-info-index')->withFail('Station infomation name is exist. Pleast check.');
        ...
        if ($info->save())
            return redirect()->route('stations-info-index')->withSuccess('Station infomation updated...');
        else
            return redirect()->route('stations-info-index')->withFail('Something is wrong. Please try again.');
    }
    ...
    public function destroy(string $id = null)
    {
        if ($id) {
            ...
            $info->forceDelete();
            return redirect()->route('stations-info-index')->withSuccess('Station infomation deleted...');
        }
        return redirect()->route('stations-info-index')->withFail('Something is wrong. Please try again.');
    }
