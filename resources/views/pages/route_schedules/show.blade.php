...
@section('page-title')
<h2 class="ms-2 mb-0" id="station-page-title"><span class="text-main-color-2">{{ $route->station_from->name }} <i class="fa-solid fa-angles-right px-2"></i>
        {{ $route->station_to->name }}</span> {{ date('H:i', strtotime($route->depart_time))}}/{{ date('H:i', strtotime($route->arrive_time))}}</h2>
    ...
    <div class="col-12 col-lg-3 p-2 ">
        ...
        <table class="table table-bordered table-align-middle">
            ...
            <tbody>
                ...
                <tr>
                    @foreach ($row as $col)
                    <td class="text-end @if (isset($routeDailyMaps[$col['date']]) && $routeDailyMaps[$col['date']] == 'Y') text-success
